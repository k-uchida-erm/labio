...
#### FR-ACT-001-2: サブタスク作成

**説明**: 既存のActivityのサブタスクを作成できる

**受入基準**:

- [ ] 親ActivityのAdd Sub Activityボタンから作成できる
- [ ] サブタスクは親Activityの下にインデント表示される
- [ ] サブタスクは親Activityのトグルで開閉できる
- [ ] サブタスク作成時、親Activityが自動的に展開される
- [ ] サブタスクも通常のActivityと同様に編集・削除可能

#### FR-ACT-001-3: タイプごとのメタデータ保持とUI出し分け

**説明**: タスク以外（質問/実験/レビューなど）を表現するため、Activityはタイプごとに必要な追加フィールドを持つ。

**受入基準**:

- [ ] `activities.metadata (jsonb)` にタイプ固有の値を保存する（例: `{"question":{"requested_reviewer_id":"..."}}`）
- [ ] ActivityItemは `type` + `metadata` を受け取り、ベース行レイアウトを維持しつつタイプ別のメタ情報ブロック（TaskActivityMetaなど）を描画する
- [ ] 新しいタイプを追加する際は `metadata` に構造を定義し、対応するメタ描画コンポーネントを追加するだけで拡張できる
- [ ] Questionなど期日不要のタイプでもUI崩れがない（不要なDue/Assigneeバッジは描画しない）

**備考**: Activity一覧は共通の `ActivityItem` を親として使い、個別のタイプ固有UIは `renderTypeMeta` ハンドラで切り替える。DB/フロント双方がメタデータ経由で拡張できることを保証する。
...
| `parent_id`       | UUID            | -    | 親ActivityのID（サブタスクの場合）         |
