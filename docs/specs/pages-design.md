...
- 右側からスライドインするサイドパネル形式で表示（画面幅の約50%）
- アクティビティタイプごとに異なるテンプレートで表示
  - Task: 標準的なタスク詳細表示
  - Experiment: 実験ノート用の表示（将来拡張）
  - Question: 質問用の表示（将来拡張）
  - Review: レビュー依頼用の表示（将来拡張）
  - Meeting: 面談・ゼミ用の表示（将来拡張）
  - Note: メモ用の表示（将来拡張）
- Activity編集（将来実装）
- タグ管理（追加、削除）（将来実装）
- コメント一覧表示（将来実装）
- コメント追加・編集・削除（将来実装）
- 添付ファイル一覧表示（将来実装）
- 添付ファイルアップロード・削除（将来実装）

**実装方式**: Supabase Client SDK（表示）、Server Actions（編集、将来実装）

**Phase**: 1（基本表示）、2（コメント・添付ファイル追加）

**URL例**: `/tokyo-univ-ai-lab-a3f2/PINN/1`（表示ID: `PINN-1`）

**UI/UX**:

- アクティビティ一覧のタイトルをクリックすると詳細ページに遷移
- サイドパネルは右側からスムーズにスライドイン（300ms transition）
- オーバーレイをクリックまたは閉じるボタンでプロジェクトページに戻る
